...
public class NAvLearningViewTargets : ExperimentTask {

    [Header("Task-specific Properties")]

    public ObjectList startObjects;
    private ObjectList navlearningObjects;
    ...
    public override void startTask () {
        ...
        for (int i = 0; i < NavLearningSetup.environmentList.Count; i++)
        {
            //hopefully add the right object
            //navlearningObjects.objects.Add(startObjects.objects[NavLearningSetup.environmentList[i]]);
            //Debug.LogError(NavLearningSetup.environmentList[i] + " " + navlearningObjects.objects[i].name);
            //Debug.LogError(startObjects.objects[i]);
            Debug.LogError(startObjects.objects[NavLearningSetup.environmentList[i]].name);

        }

        //navlearningObjects.objects.Add(objects);

		TASK_START();
		current = startObjects.objects[NavLearningSetup.environmentList[count]];
	...
	public override bool updateTask () {
		...
		if (current) {
			...
			if ( Experiment.Now() - rotation_start >= interval)  {
                // startObjects.objects[NavLearningSetup.environmentList[i]].gameObject
                if (count < NavLearningSetup.environmentList.Count)
                {
                    rotation_start = Experiment.Now();
                    rotation_start_float = rotation_start / 1f;
	...
	public void initCurrent() {
        ...
        //current.transform.position = avatar.transform.position + avatar.transform.forward;
        // avatar.transform.position = current.transform.GetChild(0).transform.position - new Vector3 (0.0f, 0.0f, 100.0f);
        avatar.transform.position = current.transform.GetChild(0).transform.position - ((current.transform.GetChild(0).transform.position - current.transform.Find("SnapPoint").position).normalized *100);
        avatar.transform.LookAt(current.transform.Find("SnapPoint").position, Vector3.up);
