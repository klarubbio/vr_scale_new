...
public class NavLearningMoveSpawn : ExperimentTask {
    ...
    public override void TASK_START()
    {
        ...
        //Nav learning specific functionality code
        destination = destinations.objects[NavLearningSetup.currentStart];

        position = start.transform.position;
        rotation = start.transform.eulerAngles;


        // Set the position to the snap point, but ignore the y-axis (just 2d position on map)
        Vector3 tempPos = start.transform.position;
        tempPos.x = destination.transform.Find("SnapPoint").position.x;
        if (!ignoreY)
        {
            tempPos.y = destination.transform.Find("SnapPoint").position.y;
        }
        tempPos.z = destination.transform.Find("SnapPoint").position.z;
        //add the forward position to spawn the player infront of the object
	...
	public override void TASK_END() {
		base.endTask();
            ...
            //Nav Learning specific functionality
            destination = destinations.objects[NavLearningSetup.currentStart];
